...
export default function AdminDashboard() {
  ...
  async function handleResetPassword(userId, username) {
    ...
    if (res.ok) {
      alert('ਪਾਸਵਰਡ ਰੀਸੈੱਟ ਹੋ ਗਿਆ (131313)')
    }
  }
    ...
    <div className="min-h-screen" style={{ paddingTop: '72px' }}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8 animate-fadeInUp">
          <div>
            <h1 className="text-3xl font-bold text-white mb-1">ਪ੍ਰਬੰਧਕ ਪੈਨਲ</h1>
            <p style={{ color: 'rgba(255,255,255,0.4)' }}>ਸਾਰੇ ਵਰਤੋਂਕਾਰ ਅਤੇ ਤਰੱਕੀ ਦੇਖੋ</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'ਵਰਤੋਂਕਾਰ', value: stats.users, emoji: '👥', color: '#a78bfa' },
            { label: 'ਵਿਸ਼ੇ', value: stats.topics, emoji: '📚', color: '#fbbf24' },
            { label: 'ਦ੍ਰਿਸ਼ਟੀਕੋਣ', value: stats.angles, emoji: '🔭', color: '#34d399' },
            { label: 'ਸ਼ਬਦ', value: stats.shabads, emoji: '📖', color: '#f472b6' },
          ].map((s, i) => (
        ...
        <div className="glass-card-static rounded-2xl overflow-hidden animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
          <div className="p-5 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            <h2 className="text-lg font-bold text-white">ਸਾਰੇ ਵਰਤੋਂਕਾਰ</h2>
          </div>

          {users.length === 0 ? (
            <div className="p-12 text-center">
              <div className="text-4xl mb-3">👤</div>
              <p style={{ color: 'rgba(255,255,255,0.4)' }}>ਅਜੇ ਕੋਈ ਵਰਤੋਂਕਾਰ ਨਹੀਂ ਜੋੜਿਆ</p>
            </div>
          ) : (
            <div className="divide-y" style={{ divideColor: 'rgba(255,255,255,0.05)' }}>
              {users.map(user => (
                <div key={user.id} className="p-4 flex items-center gap-4 transition-colors hover:bg-white hover:bg-opacity-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>@{user.username}</p>
                  </div>
                  <div className="flex items-center gap-4 text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    <span className="hidden sm:block">{userStats[user.id]?.angles || 0} ਦ੍ਰਿਸ਼ਟੀਕੋਣ</span>
                    <span className="hidden sm:block">{userStats[user.id]?.shabads || 0} ਸ਼ਬਦ</span>
                    <span className="font-bold" style={{ color: '#fbbf24' }}>{user.score || 0} ✦</span>
                  </div>
      ...
      {showAddUser && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowAddUser(false)}>
          <div className="modal-box">
            <h3 className="text-xl font-bold text-white mb-1">ਨਵਾਂ ਵਰਤੋਂਕਾਰ</h3>
            <p className="text-sm mb-6" style={{ color: 'rgba(255,255,255,0.4)' }}>
              ਡਿਫਾਲਟ ਪਾਸਵਰਡ: <span className="font-mono text-white">131313</span>
            </p>
            <form onSubmit={handleAddUser} className="space-y-4">
              <div>
                <label className="block text-sm mb-1.5" style={{ color: 'rgba(255,255,255,0.6)' }}>ਪੂਰਾ ਨਾਮ</label>
                <input className="input-royal" placeholder="ਨਾਮ ਲਿਖੋ" value={newFullName} onChange={e => setNewFullName(e.target.value)} required />
              </div>
              <div>
                <label className="block text-sm mb-1.5" style={{ color: 'rgba(255,255,255,0.6)' }}>ਵਰਤੋਂਕਾਰ ਨਾਮ (ਲਾਗਇਨ ID)</label>
              ...
              {addUserMsg && (
                <div className="rounded-xl px-4 py-3 text-sm text-center"
                  style={{
                    background: addUserMsg.includes('ਸਫਲ') ? 'rgba(52,211,153,0.12)' : 'rgba(239,68,68,0.12)',
                    border: `1px solid ${addUserMsg.includes('ਸਫਲ') ? 'rgba(52,211,153,0.3)' : 'rgba(239,68,68,0.3)'}`,
                    color: addUserMsg.includes('ਸਫਲ') ? '#6ee7b7' : '#fca5a5',
                  }}>
                  {addUserMsg}
                </div>
              )}
